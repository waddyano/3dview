...
bool CBitmapFont::Load(const TCHAR *fname)
{
    fstream in;
    ...
    char *img=new char[(ImgX*ImgY)*(bpp/8)];
...
void CBitmapFont::Print(const char* Text)
{
    ...
    {
        int Row=(Text[Loop]-Base)/RowPitch;
        int Col=(Text[Loop]-Base)-Row*RowPitch;
...
void CBitmapFont::ezPrint(const char *Text, int x, int y)
{
    if (Text[0] == '\0')
        return;
...
int CBitmapFont::GetWidth(const char* Text)
{
    int Size = 0;
